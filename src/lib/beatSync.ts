...
export class BeatSyncTools {
  ...
  quantizeTime(time: number, options: BeatSyncOptions): number {
    ...
    switch (options.quantizeLevel) {
      case "1/1":
        subdivisionInterval = beatInterval * 4;
        break; // whole note
      case "1/2":
        subdivisionInterval = beatInterval * 2;
        break; // half note
      case "1/4":
        subdivisionInterval = beatInterval;
        break; // quarter note
      case "1/8":
        subdivisionInterval = beatInterval / 2;
        break; // eighth note
    ...
    const subdivisions = Math.round(
      (time - nearestBeat.time) / subdivisionInterval
    );
  ...
  nudgeTime(
    ...
    quantizeLevel: BeatSyncOptions["quantizeLevel"] = "1/4"
  ): number {
    const bpm = this.estimateBPM();
    const beatInterval = 60 / bpm;
  ...
  detectAutoCutPoints(
    ...
    for (const beat of beatsInRange) {
      ...
      if (
        beat.type === "downbeat" ||
        beat.strength >= options.energyThreshold
      ) {
...
export const generateMockBeats = (
  duration: number,
  bpm: number = 120
): BeatInfo[] => {
  const beatInterval = 60 / bpm;
