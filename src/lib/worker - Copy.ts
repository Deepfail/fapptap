...
export class PythonWorker {
  ...
  async runStage(
    ...
    if (!isTauriAvailable()) {
      const error = "Python worker execution is only available in desktop mode";
      logger.error(error);
      throw new Error(error);
    }

    logger.debug('[PythonWorker] isTauriAvailable() = true, proceeding with sidecar spawn');
  ...
  logger.info(`Running worker stage: ${stage} with args:`, workerArgs);
  logger.debug('[PythonWorker] full arg object:', args);
    logger.debug("Raw args object:", args);
    ...
    return new Promise((resolve, reject) => {
      ...
      this.currentCommand.on("close", (data: any) => {
        logger.info(`Worker stage ${stage} completed with code:`, data.code);
        if (data.code === 0) {
          resolve();
        } else {
      ...
      this.currentCommand.on("error", (error: any) => {
        logger.error(`Worker stage ${stage} error:`, error);
        ...
        try {
          ...
          for (const line of lines) {
            try {
              ...
              logger.info(`Worker stdout: ${line}`);
            }
          }
        } catch (e) {
          // Handle UTF-8 decode errors gracefully
          logger.warn(`Worker stdout decode error:`, e);
        }
      });
      ...
      this.currentCommand.stderr.on("data", (data: any) => {
        try {
          ...
          logger.warn(`Worker stderr: ${text}`);
          // Log sidecar output using our dedicated function
          logSidecarOutput("worker", "", text);
        } catch (e) {
          // Handle UTF-8 decode errors gracefully
          logger.warn(`Worker stderr decode error:`, e);
        }
      });
