...
export const SpeedRampEditor = () => {
  ...
  const addKeyframe = (time: number, speed: number = 1) => {
    const newKeyframes = [...speedRamp.keyframes, { time, speed }].sort(
      (a, b) => a.time - b.time
    );
  ...
  const snapToNearestBeat = (clipTime: number) => {
    ...
    const absoluteTime =
      selectedItem.start + clipTime * (selectedItem.out - selectedItem.in);
    const closestBeat = mockBeats.reduce((prev, curr) =>
      Math.abs(curr - absoluteTime) < Math.abs(prev - absoluteTime)
        ? curr
        : prev
    );
    ...
    const relativeTime =
      (closestBeat - selectedItem.start) / (selectedItem.out - selectedItem.in);
  ...
  const anchorToBeat = (index: number) => {
    ...
    const absoluteTime =
      selectedItem.start + snappedTime * (selectedItem.out - selectedItem.in);
    const closestBeat = mockBeats.reduce((prev, curr) =>
      Math.abs(curr - absoluteTime) < Math.abs(prev - absoluteTime)
        ? curr
        : prev
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <input
            ...
            onChange={(e) =>
              updateSpeedRamp({ ...speedRamp, enabled: e.target.checked })
            }
            className="w-4 h-4"
          />
        </div>
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <div className="h-24 bg-slate-900 rounded border relative overflow-hidden">
                <svg className="w-full h-full">
                  ...
                  {speedRamp.keyframes.map((kf, i) => {
                    ...
                    return (
                      <circle
                        ...
                        onClick={() =>
                          setSelectedKeyframe(selectedKeyframe === i ? null : i)
                        }
                      />
                    );
                  })}
                  ...
                  {mockBeats.map((beat) => {
                    ...
                    const relativeTime =
                      (beat - selectedItem.start) /
                      (selectedItem.out - selectedItem.in);
              ...
              <div className="text-xs text-muted-foreground">
                Red lines show beat positions • Click points to select • Blue
                line shows speed curve
              </div>
            </div>

            {/* Keyframe Editor */}
            {selectedKeyframe !== null &&
              selectedKeyframe < speedRamp.keyframes.length && (
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    const quantizedKeyframes = speedRamp.keyframes.map(
                      (kf) => ({
                        ...kf,
