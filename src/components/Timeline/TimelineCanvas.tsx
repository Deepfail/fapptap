...
import { snapTime, formatTime } from '../../utils/timelineUtils';
...
export function TimelineCanvas({ width, height, className }: TimelineCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Draw beat grid
    drawBeats(ctx, beats, startTime, endTime, pixelsPerSecond, scrollLeft, height);
    
    // Draw cuts
    drawCuts(ctx, cuts, selectedCutId, startTime, endTime, pixelsPerSecond, scrollLeft, height);
    
    // Draw playhead
    drawPlayhead(ctx, currentTime, pixelsPerSecond, scrollLeft, height);

  }, [width, height, duration, currentTime, beats, cuts, selectedCutId, pixelsPerSecond, scrollLeft]);
...
function drawBeats(
  ...
  const visibleBeats = beats.filter(b => b.time >= startTime && b.time <= endTime);
...
function drawCuts(
  ...
  const visibleCuts = cuts.filter(cut => 
    cut.end >= startTime && cut.start <= endTime
  );
