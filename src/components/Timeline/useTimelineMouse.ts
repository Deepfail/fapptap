...
export function useTimelineMouse(
  containerRef: React.RefObject<HTMLElement | null>
): TimelineMouseHandlers {
  ...
  const detectHitTarget = useCallback(
    (x: number, y: number): { type: string; cutId?: string } => {
      // const time = getTimeFromX(x); // Unused for now
      ...
      switch (mouseState.current.dragType) {
        ...
        case "move":
          if (mouseState.current.dragCutId) {
            ...
            if (cut) {
              ...
              // const newEnd = constrainTime(cut.end + deltaTime, duration); // Unused
        ...
        case "resize-left":
          if (mouseState.current.dragCutId) {
            const cut = cuts.find((c) => c.id === mouseState.current.dragCutId);
            if (cut) {
              const { start } = preventZeroLengthCuts(snappedTime, cut.end);
              updateCut(mouseState.current.dragCutId, {
                start: constrainTime(start, duration),
              });
            }
          }
          break;
        ...
        case "resize-right":
          if (mouseState.current.dragCutId) {
            const cut = cuts.find((c) => c.id === mouseState.current.dragCutId);
            if (cut) {
              const { end } = preventZeroLengthCuts(cut.start, snappedTime);
              updateCut(mouseState.current.dragCutId, {
                end: constrainTime(end, duration),
              });
            }
          }
          break;
  ...
  const onMouseUp = useCallback(
    (_event: React.MouseEvent) => {
      ...
      if (
        mouseState.current.dragType === "create" &&
        mouseState.current.tempCut
      ) {
        ...
        if (Math.abs(end - start) > 0.1) {
          // Only create if meaningful duration
