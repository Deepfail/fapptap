...
export const DragGhost = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('fapptap-drag-start', onStart as EventListener);
  ...
  return (
    <div style={{ position: 'fixed', left: pos.x + 12, top: pos.y + 12, pointerEvents: 'none', zIndex: 9999 }}>
      <div className="flex items-center gap-2 bg-slate-800/90 text-white px-3 py-2 rounded shadow-lg">
        {payload.thumbnail ? (
          <img src={payload.thumbnail} className="w-12 h-8 object-cover rounded" alt={payload.name} />
        ) : null}
