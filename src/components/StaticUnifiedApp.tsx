...
export function StaticUnifiedApp() {
  ...
  const prevPage = () =>
    setState((p) => ({ ...p, currentPage: Math.max(0, p.currentPage - 1) }));
  const nextPage = () =>
    setState((p) => ({ ...p, currentPage: p.currentPage + 1 }));
  const canPrevPage = state.currentPage > 0;
  const canNextPage =
    (state.currentPage + 1) * state.itemsPerPage < state.totalFiles;
                        ...
                        <div className="absolute top-1 right-1 flex flex-col gap-1">
                          {file.type === "video" && (
                            <>
                              ...
                              {(() => {
                                const probeStatus = getFileProbeStatus(
                                  file.path
                                );
                                const probeColor =
