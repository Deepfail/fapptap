...
export function LibraryPane() {
  ...
  const pendingJobs = jobs.filter(job => 
    job.type === 'probe' || job.type === 'thumbnail'
  );
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b border-slate-700">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          ...
          <Button size="sm" onClick={handleSelectClipsDirectory} variant="outline">
      ...
      {(isScanning || pendingJobs.length > 0) && (
        <div className="p-4 border-b border-slate-700">
          <div className="text-sm text-slate-300 mb-2">
            {isScanning ? 'Scanning directory...' : `Processing ${pendingJobs.length} files...`}
          </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        {sortedFiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-slate-400">
            ...
            <p className="text-xs opacity-75">Select a clips directory to get started</p>
          </div>
        ) : (
          <div className={`p-3 ${
            viewMode === 'grid' 
