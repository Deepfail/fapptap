...
function main() {
  ...
  if (!/trigger_header:\s*"mode: agent"/m.test(yaml))
    fail("trigger_header missing or incorrect");
  if (!/response_mode:\s*final-only/m.test(yaml))
    fail("response_mode: final-only missing for AUTO");
